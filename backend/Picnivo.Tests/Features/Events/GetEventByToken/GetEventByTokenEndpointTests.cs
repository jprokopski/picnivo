...
public class GetEventByTokenEndpointTests(ApiFixture fixture)
{
    ...
    public async Task JoinVoteThenGet_ReturnsTalliesBestDateAndYou()
    {
        ...
        var dateOptionId = await db.Events.Where(e => e.Token == token).SelectMany(e => e.DateOptions).Select(d => d.Id).FirstAsync();

        // Act
        var joinResponse = await ctx.ApiClient.JoinEventAsync(token, new JoinEventRequest { DisplayName = "Alice" });
        await ctx.ApiClient.CastVotesAsync(token, joinResponse.ParticipantId,
    ...
    private static async Task<string> SeedEventAsync(IServiceProvider services, Guid organizerId, int dateOptionCount = 1)
    {
        using var scope = services.CreateScope();
        var db = scope.ServiceProvider.GetRequiredService<PicnivoDbContext>();
        ...
        var @event = new Event
        {
            ...
            DateOptions = Enumerable.Range(1, dateOptionCount)
