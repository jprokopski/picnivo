...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "pain type"
      decision: "coordination overhead + decision paralysis + data trapped in chat"
    - topic: "insight"
      decision: "existing tools are too heavy (public/large events) and too narrow (each does one piece, nobody stitches them)"
    - topic: "primary persona"
      decision: "the friend-group organizer — the one who makes plans happen"
    - topic: "auth strategy"
      decision: "organizer logs in (email/password or OAuth); participants join via shareable link, no account required"
    - topic: "role model"
      decision: "two roles — organizer (create/manage events) and participant (vote, claim items, view)"
    - topic: "mvp scope"
      decision: "9-step flow; email reminders with weather cut to v2"
    - topic: "timeline"
      decision: "3 weeks after-hours; scoped down to make it comfortable"
    - topic: "domain rule"
      decision: "coordination convergence — reduces multi-person, multi-decision loop to a single shared state"
    - topic: "real-time updates"
      decision: "updates on page load for MVP; real-time (WebSockets) deferred to v2"
    - topic: "item list ownership"
      decision: "both organizer and participants can add items; organizer can remove any"
  frs_drafted: 12
